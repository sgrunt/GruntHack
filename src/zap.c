	...
	case SPE_FORCE_BOLT:
		...
		if (resists_magm(mtmp)) {	/* match effect on player */
			...
			if (is_stone(mtmp->data)) {
				...
				mtmp->mhp = -1;
				mtmp = xkilled(mtmp, 2, AD_SPEL); /* no corpse */
...
#define oresist_disintegration(obj) \
		(objects[obj->otyp].oc_oprop == DISINT_RES || \
		 ...
		 obj == m_amulet)
			...
			if (type < 0)
			    mon = monkilled(mon, (char *)0, -AD_RBRE);
			else
			    mon = xkilled(mon, 2, -AD_RBRE);
		    } else if(mon->mhp < 1) {
			if(type < 0)
			    mon = monkilled(mon, fltxt, AD_RBRE);
			else
			    mon = killed(mon, AD_RBRE);
		    } else {
			if (!otmp) {
...
int damage, tell;
{
	...
	if (damage) {
	    mtmp->mhp -= damage;
	    if (mtmp->mhp < 1) {
		if(m_using) mtmp = monkilled(mtmp, "", AD_RBRE);
		else mtmp = killed(mtmp, AD_RBRE);
	    }
	}
	return(resisted);
}
